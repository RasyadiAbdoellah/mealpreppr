...
const IngredientItem = (ingredient, index, fields) => {
  ...
  if (fields.length > 1)
    ...
    ); //if Ingredients array is more than 1, and current element is not last, show remove button
  if (fields.length === 0 || index === fields.length - 1)
    add = (
      <button type="button" onClick={() => fields.push()}>
        {' '}
        +{' '}
      </button>
    ); //if Ingredient array is empty, or current element is last, show add button
...
const IngredientFieldArray = props => {
  ...
  return <ul className="ingredients">{fields.map(IngredientItem)}</ul>;
};
...
class RecipeFormClass extends React.Component {
  ...
  submit = values => {
    ...
    if (id === 'new') {
      return axios
        .post(API_URL + '/recipes', data, { headers })
        .then(res => {
          addRecipe(res);
      ...
      return axios
        .patch(API_URL + '/recipes/' + id, data, { headers })
        .then(res => {
          updateRecipe(res);
...
function mapStateToProps(state, { id }) {
  return id === 'new'
    ? { initialValues: {} }
    : { initialValues: getRecipeById(state, id) };
}
